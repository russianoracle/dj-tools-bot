...
class BuildupDetectorML(BaseTask):
    ...
    def train_model(
        ...
        """
        DEPRECATED: Use src.training.trainers.BuildupDetectorTrainer instead.

        Training logic has been moved to separate module following SRP.
        ...
        Usage:
            ...
            model_bundle, metrics = trainer.train(features_df)
            trainer.save(model_bundle, save_path)
        """
        import warnings
        warnings.warn(
            "BuildupDetectorML.train_model() is deprecated. "
        ...
        from src.training.trainers import BuildupDetectorTrainer
        trainer = BuildupDetectorTrainer()
        model_bundle, metrics = trainer.train(features_df, label_column)
        if save_path:
            trainer.save(model_bundle, save_path)
        self._model = model_bundle['model']
        self._label_encoder = model_bundle['label_encoder']
        return model_bundle['model'], metrics