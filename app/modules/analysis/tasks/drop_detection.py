...
class DropDetectionTask(BaseTask):
    ...
    def execute(self, context: AudioContext) -> DropDetectionResult:
        ...
        try:
            ...
            if self.use_multiband and context.stft_cache is not None:
                # Get mel spectrogram from STFTCache (librosa centralized)
                S_mel = context.stft_cache.get_mel(n_mels=self.n_mels)
                S_mel_db = 10.0 * np.log10(S_mel + 1e-10)  # power_to_db equivalent
                S_mel_db -= np.max(S_mel_db)  # ref=np.max
