...
Usage:
    from src.services import ProfilingService
...
class ProfilingService:
    ...
    def _aggregate_profile(
        ...
        """
        Aggregate individual set metrics into DJ profile.
        """
        import numpy as np
        ...
        for m in metrics:
            if m.energy_arc:
                energy_shapes.append(m.energy_arc.arc_shape)
                total_duration += m.energy_arc.duration_min / 60.0

            if m.drop_pattern:
                drops_per_hour.append(m.drop_pattern.drops_per_hour)

            if m.tempo_distribution:
                tempos.append(m.tempo_distribution.tempo_mean)

        # Aggregate
        avg_drops = np.mean(drops_per_hour) if drops_per_hour else 0.0
        tempo_range = (min(tempos), max(tempos)) if tempos else (0, 0)
        dominant_tempo = int(np.median(tempos)) if tempos else 0

        # Most common energy shape
        if energy_shapes:
            from collections import Counter
            shape_counts = Counter(energy_shapes)
            avg_shape = shape_counts.most_common(1)[0][0]
        else:
            avg_shape = "unknown"
        ...
        return DJProfile(
            ...
            preferred_keys=[],  # TODO: aggregate key analysis
