...
def _stft_numpy(y: np.ndarray, n_fft: int = 2048, hop_length: int = 512) -> np.ndarray:
    ...
    Returns:
        Complex STFT matrix (n_freq, n_frames)
    """
    ...
    # Preallocate output
    n_freq = n_fft // 2 + 1
    D = np.zeros((n_freq, n_frames), dtype=np.complex64)
    ...
    fft_result = np.fft.rfft(windowed, axis=1).T  # Transpose to (n_freq, n_frames)

    return fft_result.astype(np.complex64)
...
class STFTCache:
    ...
    def get_hpss(self) -> tuple:
        ...
        if cache_key not in self._feature_cache:
            ...
            y_h, y_p = get_harmonic_percussive(self._feature_cache['_y'])
            self._feature_cache[cache_key] = (
    ...
    def get_cqt(self, n_bins: int = 84, bins_per_octave: int = 12) -> np.ndarray:
        ...
        if cache_key not in self._feature_cache:
            ...
            C = get_cqt(self._feature_cache['_y'], self.sr, self.hop_length,
