...
class DropDetectorML(BaseTask):
    ...
    def train_model(
        ...
        """
        DEPRECATED: Use src.training.trainers.DropDetectorTrainer instead.
        ...
        import warnings
        warnings.warn(
            "DropDetectorML.train_model() is deprecated. "
        ...
        from src.training.trainers import DropDetectorTrainer
        trainer = DropDetectorTrainer()
        model, metrics = trainer.train(features_df, label_column)
        if save_path:
            trainer.save(model, save_path)
        self._model = model
