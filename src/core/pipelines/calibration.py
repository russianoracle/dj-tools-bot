...
Uses SetAnalysisPipeline for loading + CacheManager for feature caching.
After first run, all features are cached and optimization is ~100x faster.

Usage:
    from src.core.pipelines import CalibrationPipeline, MixingStyle
...
from .base import Pipeline, PipelineStage, PipelineContext, LoadAudioStage, ComputeSTFTStage
from .cache_manager import CacheManager
from ..tasks import TransitionDetectionTask
from ..config import MixingStyle, TransitionConfig
...
class LoadAndCacheFeaturesStage(PipelineStage):
    ...
    Uses CacheManager for persistent caching:
    ...
    def __init__(self, cache_manager: CacheManager, sr: int = 22050, verbose: bool = False, num_workers: int = 1):
    ...
    def _load_one_set(self, audio_path: str, ground_truth_dict: Dict, pbar=None) -> Optional[CachedSetFeatures]:
        ...
        # Compute file hash
        file_hash = self.cache_manager.compute_file_hash(str(path))

        # Try cache first
        if self.cache_manager.has_all_derived_features(file_hash, CACHED_FEATURE_NAMES):
            if self.verbose:
                print(f"[CACHE] {set_id}: checking cache...", flush=True)

            start = time.time()
            features = self.cache_manager.get_derived_features_batch(file_hash, CACHED_FEATURE_NAMES)

            if self.verbose:
                print(f"[CACHE] {set_id}: loaded from cache [{time.time() - start:.2f}s]", flush=True)

            # Load metadata
            metadata_path = self.cache_manager.stft_dir / f"{file_hash}_metadata.json"
        ...
        for k, v in features.items():
            if isinstance(v, np.ndarray):
                ...
                feature_arrays[k] = v
        self.cache_manager.save_derived_features_batch(file_hash, feature_arrays)
...
class OptimizeTransitionParamsStage(PipelineStage):
    ...
    def process(self, context: PipelineContext) -> PipelineContext:
        ...
        def objective(params):
            ...
            if self.verbose:
                print(f"\n[Iteration {n_calls[0]}] Testing parameters:")
                print(f"  gap={params[0]:.1f}, thr={params[1]:.3f}, bass={params[2]:.2f}, sigma={params[3]:.1f}")

            tp, fp, fn = self._run_detection(cached_sets, params)
            f1, precision, recall = self._compute_metrics(tp, fp, fn)

            if self.verbose:
                print(f"  Results: TP={tp}, FP={fp}, FN={fn} → P={precision:.1%}, R={recall:.1%}")
            ...
            if f_beta > best_f1[0]:  # Using best_f1 var name for compatibility
                ...
                if self.verbose:
                    print(f"  ✨ NEW BEST! F-beta={f_beta:.3f} (P={precision:.1%}, R={recall:.1%})")
        ...
        if self.verbose:
            print(f"\nOptimizing {self.mixing_style.value} (max {self.max_iter} iter)...")
            print(f"Starting from current config")
    ...
    def _run_detection(self, cached_sets: List[CachedSetFeatures], params: np.ndarray) -> Tuple[int, int, int]:
        ...
        # Process all sets with same task instance
        for i, cs in enumerate(cached_sets, 1):
            if self.verbose:
                print(f"    [{i}/{len(cached_sets)}] Processing {cs.set_id}...", end=" ", flush=True)
            ...
            if self.verbose:
                print(f"detected {len(detected)} mixins (GT: {len(cs.ground_truth)})", flush=True)
