...
# Import centralized STFTCache from core
from ..core.primitives.stft import STFTCache
...
class M2MixinMixoutDetector:
    ...
    def _analyze_audio(self, y: np.ndarray, sr: int,
        ...
        # ============================================================
        # PHASE 1: Core computations (compute ONCE)
        # ============================================================

        print("  Computing STFT...")

        # Single STFT - foundation for all features
        S = np.abs(librosa.stft(y, n_fft=n_fft, hop_length=hop))
        S_power = S ** 2
        n_frames = S.shape[1]

        # Frequency axis (compute once)
        self._freqs = librosa.fft_frequencies(sr=sr, n_fft=n_fft)
        ...
        # Spectral centroid (filter position proxy)
        ...
        # Spectral rolloff (another filter indicator)
        ...
        # Onset strength
        onset_env = librosa.onset.onset_strength(S=librosa.power_to_db(S_power), sr=sr)[:n_frames]
