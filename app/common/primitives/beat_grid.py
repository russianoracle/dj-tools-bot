...
def compute_beat_grid(
    ...
    beat_strengths = np.zeros(len(beat_frames), dtype=np.float32)
    beat_strengths[:len(valid_beats)] = onset_env[valid_beats]

    # Build beat list (VECTORIZED - compute all positions at once)
    beats_per_phrase = beats_per_bar * bars_per_phrase
    n_beats = len(beat_frames)

    # Vectorized position computation
    indices = np.arange(n_beats, dtype=np.int32)
    adjusted_idx = indices - downbeat_offset
    bar_positions = (adjusted_idx % beats_per_bar) + 1
    phrase_positions = (adjusted_idx % beats_per_phrase) + 1

    # Convert to list of BeatInfo objects
    beats = [
        BeatInfo(
            time_sec=float(beat_times[i]),
            frame_idx=int(beat_frames[i]),
            bar_position=int(bar_positions[i]),
            phrase_position=int(phrase_positions[i]),
            strength=float(beat_strengths[i])
        )
        for i in range(n_beats)
    ]

    # Build bar list (VECTORIZED - precompute all indices and times)
    n_full_bars = (len(beats) - downbeat_offset) // beats_per_bar
    if n_full_bars > 0:
        # Vectorized computation of bar parameters
        bar_indices_arr = np.arange(n_full_bars, dtype=np.int32)
        start_beat_indices = downbeat_offset + bar_indices_arr * beats_per_bar
        end_beat_indices = start_beat_indices + beats_per_bar

        # Filter valid bars
        valid_mask = end_beat_indices <= len(beats)
        valid_bar_indices = bar_indices_arr[valid_mask]

        # Extract beat times as array for vectorized access
        beat_times_arr = np.array([b.time_sec for b in beats], dtype=np.float32)
        beat_duration = 60.0 / tempo if tempo > 0 else 0.5

        bars = []
        for bar_idx in valid_bar_indices:
            start_beat_idx = int(downbeat_offset + bar_idx * beats_per_bar)
            end_beat_idx = start_beat_idx + beats_per_bar
            beat_idx_list = list(range(start_beat_idx, end_beat_idx))

            start_time = float(beat_times_arr[start_beat_idx])
            end_time = float(beat_times_arr[end_beat_idx]) if end_beat_idx < len(beats) else float(beat_times_arr[end_beat_idx - 1] + beat_duration)

            bars.append(BarInfo(
                index=int(bar_idx),
                start_time=start_time,
                end_time=end_time,
                beat_indices=beat_idx_list,
                phrase_idx=int(bar_idx // bars_per_phrase),
                bar_in_phrase=int((bar_idx % bars_per_phrase) + 1)
            ))
    else:
        bars = []

    # Build phrase list (VECTORIZED - precompute all parameters)
    n_full_phrases = len(bars) // bars_per_phrase
    if n_full_phrases > 0:
        # Extract bar times as arrays
        bar_starts_arr = np.array([b.start_time for b in bars], dtype=np.float32)
        bar_ends_arr = np.array([b.end_time for b in bars], dtype=np.float32)
        ...
        for phrase_idx in range(n_full_phrases):
            ...
            bar_idx_list = list(range(start_bar_idx, end_bar_idx))
            start_time = float(bar_starts_arr[start_bar_idx])
            end_time = float(bar_ends_arr[end_bar_idx - 1])

            phrases.append(PhraseInfo(
                index=phrase_idx,
                start_time=start_time,
                end_time=end_time,
                bar_indices=bar_idx_list,
