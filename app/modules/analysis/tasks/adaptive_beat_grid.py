...
from app.common.primitives.beat_grid import BeatGridResult, BeatInfo, BarInfo, PhraseInfo
...
class AdaptiveBeatGridTask(BaseTask):
    ...
    def execute_with_ground_truth(
        ...
        # First segment: from music_start to first drop (vectorized)
        if drop_times[0] > music_start:
            first_interval = drop_times[0] - music_start
            # Vectorized: find valid N in range [4, 20)
            n_range = np.arange(4, 20)
            tempos = n_range * self.beats_per_phrase * 60 / first_interval
            valid_mask = (tempos >= self.min_tempo) & (tempos <= self.max_tempo)
            if np.any(valid_mask):
                first_valid_idx = np.argmax(valid_mask)
                n = n_range[first_valid_idx]
                tempo = tempos[first_valid_idx]
                phrase_dur = first_interval / n
                # Vectorized phrase boundaries
                phrase_bounds = music_start + np.arange(n) * phrase_dur
                all_phrase_boundaries.extend(phrase_bounds.tolist())
        ...
        # Segments between drops (vectorized inner loops)
        for i, interval in enumerate(intervals):
            start = drop_times[i]
            end = drop_times[i + 1]

            # Vectorized: find best N in range [5, 35)
            n_range = np.arange(5, 35)
            tempos = n_range * self.beats_per_phrase * 60 / interval
            valid_mask = (tempos >= self.min_tempo) & (tempos <= self.max_tempo)
            ...
            # Find tempo closest to 132.5 (vectorized)
            valid_tempos = tempos[valid_mask]
            valid_ns = n_range[valid_mask]
            best_idx = np.argmin(np.abs(valid_tempos - 132.5))
            best_n = int(valid_ns[best_idx])
            best_tempo = float(valid_tempos[best_idx])

            # Vectorized phrase boundaries
            phrase_dur = interval / best_n
            phrase_bounds = start + np.arange(best_n) * phrase_dur
            all_phrase_boundaries.extend(phrase_bounds.tolist())
    ...
    def _detect_beats_essentia(self, y: np.ndarray, sr: int) -> Tuple[np.ndarray, float]:
        ...
        if sr != 44100:
            from app.common.primitives.audio_stft_loader import resample_audio
            y = resample_audio(y, orig_sr=sr, target_sr=44100)
            sr = 44100
    ...
    def _detect_beats_librosa(self, y: np.ndarray, sr: int) -> Tuple[np.ndarray, float]:
        """Fallback beat detection using audio_stft_loader."""
        from app.common.primitives.audio_stft_loader import get_beat_frames, frames_to_time

        tempo, beat_frames = get_beat_frames(y, sr, hop_length=512)
        beats = frames_to_time(beat_frames, sr=sr, hop_length=512)
