...
from ..primitives.rhythm import compute_onset_strength, compute_tempo
...
class TempoDistributionAnalysisTask(BaseTask):
    ...
    def execute(self, context: AudioContext) -> TempoDistributionAnalysisResult:
        ...
        # Extract tempo for each window (NO PRIOR - let each window detect independently)
        tempos = []
        ...
        for start in range(0, len(y) - window_samples, hop_samples):
            ...
            try:
                ...
                # Detect tempo independently for each window (NO PRIOR!)
                tempo_array = librosa.beat.tempo(
                    onset_envelope=onset_env,
                    sr=sr,
                    hop_length=512,
                    aggregate=None  # Return tempo for each window section
                )
