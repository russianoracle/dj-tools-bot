...
class BeatGridTask(BaseTask):
    ...
    def execute(self, audio_context: AudioContext) -> BeatGridAnalysisResult:
        ...
        downbeat_confidence = self._estimate_downbeat_confidence(beat_grid, audio_context)
    ...
    def _execute_plp_mode(self, audio_context: AudioContext) -> BeatGridAnalysisResult:
        ...
        onset_env = audio_context.stft_cache.get_onset_strength(aggregate=True)
        ...
        downbeat_confidence = self._estimate_downbeat_confidence(global_grid, audio_context)
    ...
    def _offset_grid_times(self, grid: BeatGridResult, offset_sec: float) -> BeatGridResult:
        ...
        # Create offset copies of all structural elements
        from app.common.primitives.beat_grid import BeatInfo, BarInfo, PhraseInfo

        offset_beats = [
            BeatInfo(time=b.time + offset_sec, strength=b.strength, index=b.index)
            for b in grid.beats
        ]
        offset_bars = [
            BarInfo(
                time=bar.time + offset_sec,
                beat_indices=bar.beat_indices,
                index=bar.index
            )
        ...
        offset_phrases = [
            PhraseInfo(
                time=p.time + offset_sec,
                bar_indices=p.bar_indices,
                index=p.index
            )
    ...
    def _estimate_downbeat_confidence(
        ...
        # Get onset envelope from STFTCache (librosa-based)
        onset_env = audio_context.stft_cache.get_onset_strength(aggregate=True)
