...
Usage:
    from src.services import create_analysis_service
...
class AnalysisService:
    ...
    def analyze_batch(
        ...
        # Separate cached vs uncached (using read-only cache status)
        cached_files = []
        uncached_files = []

        for f in files:
            if not force and use_cache and self.is_set_cached(str(f)):
    ...
    def _deduplicate_files(self, files: List[Path]) -> List[Path]:
        """
        Remove duplicate sets (same content, different format).
        ...
        for f in files:
            # Normalize: lowercase, remove quality suffixes, normalize separators
            name = f.stem.lower()
        ...
        # Select best format from each group
        format_priority = {'.flac': 0, '.wav': 1, '.m4a': 2, '.mp3': 3, '.aac': 4, '.opus': 5, '.ogg': 6}
        result = []
        for group_files in groups.values():
            best = min(group_files, key=lambda f: format_priority.get(f.suffix.lower(), 99))
            result.append(best)
